...
int instance_draw(Tobject ID, unsigned int *shader, Tcamera camera, unsigned int texture, int type)
{
	useShader(shader);
...
int run_camera(Tcamera *camera, GLFWwindow *window, float delta)
{
	...
	if (perspective)
	{
		glm_perspective(glm_rad(camera->fov), (float)(camera->width) / (float)(camera->height), 0.1, 100.0f, camera->projection);
	}
	else
	{
		glm_ortho(-((float)camera->width/100)/2, ((float)camera->width/100)/2, -((float)camera->height/100)/2, ((float)camera->height/100)/2, 0.1f, 100.0f, camera->projection);
	}

...
int main()
{
	...
	unsigned int cratesAmount = 10;
	mat4 cratesMatrices[10];
	int x, y, z, i, j, k;
	unsigned int ibCrates, ibTiles, ibBushes;
	size_t vec4size;
	vec3 angleDir = {0.4f, 0.6f, 0.8f};
	vec3 lightColor = {0.5f, 0.7f, 1.0f};
	...
	prepare_light(&pointShadowShader, &lightShader, 0, lightColor, 0.75f, 0.007f, 0.0002f);
	...
	setFloat(&pointShadowShader, "material.shininess", 8.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		*/
		...
		bind_texture(&pointShadowShader, floorSpecular, 2);
